...
class Simulator(gym.Env):
    ...
    def closest_curve_point2(self, pos, angle=None):
    ...
    def get_lane_pos3(self, pos, angle):
        ...
        # and the tangent at that point
        angle = angle%(2*np.pi)
        # print(f'pos:{pos}, angle:{angle}')
        point, tangent, curves = self.closest_curve_point2(pos, angle)
        if point is None:
            msg = 'Point not in lane: %s' % pos
            raise NotInLane(msg)
        ...
        return signedDist, angle_deg, pos, point, curves, np.degrees(angle)
